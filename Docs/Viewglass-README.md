...
- macOS 12.0+
- iOS app with [ViewglassServer](https://github.com/WZBbiao/ViewglassServer) integrated (for live inspection)
...
Capture high-resolution screenshots through `ViewglassServer`.
...
`viewglass` now uses the same screenshot protocol for simulator and physical
devices. It no longer shells out to `simctl` or `idevicescreenshot`.
...
Viewglass communicates with iOS apps via the Peertalk protocol over TCP:

- **Simulator ports**: 47164-47169 (localhost)
- **Device ports**: 47175-47179 — supported through `iproxy` USB forwarding
...
| Screen screenshot | Working | `screenshot screen --session 47164 -o screen.png` |
| Node screenshot | Working | `screenshot node <oid> --session 47164 -o node.png` |
...
| Discovery diagnostics | Working | `scan --verbose` |
...
Use the standalone [ViewglassServer](https://github.com/WZBbiao/ViewglassServer)
repository for all app-side integration steps.

This repo intentionally keeps the setup short to avoid documentation drift.

Only the essentials:
- integrate `ViewglassServer` in `Debug`
- keep importing `LookinServer` in app code
- if discovery fails, first verify the app actually started `LookinServer`

## Testing

```bash
swift test     # Run all 123 tests
```
