...
Viewglass (`viewglass`) 是 [Lookin](https://lookin.work) 的命令行接口 —— 一款 iOS 视图层级检查工具。它将 Lookin 的检查能力以可编程的方式暴露出来，适用于脚本自动化、CI 流水线和 AI 驱动的工作流。
...
> 注意：Homebrew 安装需要从源码编译（依赖 Xcode），首次安装需要几分钟。
> 如需更快安装，可从 [GitHub Releases](#从-github-releases-下载) 直接下载预构建二进制。
...
从 [Releases](https://github.com/WZBbiao/viewglass/releases) 页面下载对应架构的二进制：
...
```bash
make uninstall
# 或: rm /usr/local/bin/viewglass
```

## 环境要求

- macOS 12.0+
- iOS 应用需集成 [ViewglassServer](https://github.com/WZBbiao/ViewglassServer)（用于实时检查）
...
# 查询节点
viewglass query "UIButton"
viewglass query ".visible AND UILabel" --json

# 修改视图属性（实时生效）
viewglass attr set <oid> alpha 0.5
viewglass attr set <oid> text "Hello"
viewglass attr set <oid> hidden true
...
```bash
viewglass apps list              # 人类可读格式
viewglass apps list --json       # JSON 格式
viewglass apps list --mock       # 使用模拟数据（测试用）
```
...
管理检查会话。连接后会话自动持久化到 `~/.viewglass/session.json`，后续命令无需重复指定 `--session`。

```bash
viewglass session connect com.example.app     # 通过 bundle ID 连接
viewglass session connect com.example.app --json
viewglass session status                       # 查看当前会话状态
viewglass session disconnect --session <id>    # 断开连接
```
...
viewglass hierarchy dump
viewglass hierarchy dump --json
viewglass hierarchy dump --max-depth 3   # 限制展示深度
```
...
```bash
viewglass query "UILabel"
viewglass query ".visible AND UIButton" --json
viewglass query "UIButton" --count    # 只返回数量
```
...
viewglass query "(UIButton OR UILabel) AND .visible"
viewglass query "#submitButton"
viewglass query "@\"点击我\""
viewglass query "parent:UIStackView"
viewglass query "NOT UIView"
```
...
```bash
# 获取节点所有属性
viewglass attr get 4
viewglass attr get 4 --json

# 修改属性（实时生效）
viewglass attr set 4 alpha 0.5          # 修改透明度
viewglass attr set 4 hidden true        # 隐藏视图
viewglass attr set 4 text "Hello"       # 修改文本内容
viewglass attr set 4 backgroundColor "#FF0000"  # 修改背景颜色
viewglass attr set 4 cornerRadius 12    # 修改圆角

# 危险操作需要 --force
viewglass attr set 4 removeFromSuperview "" --force
```
...
viewglass console eval layoutIfNeeded --node-id 4 --json
```
...
viewglass export hierarchy -o tree.txt --format text
viewglass export hierarchy -o tree.html --format html
...
```bash
# 单项检查
viewglass diagnose overlap                # 检测重叠的可交互视图
viewglass diagnose hidden-interactive     # 检测隐藏但可交互的视图
viewglass diagnose offscreen              # 检测屏幕外的视图
...
Viewglass 通过 Peertalk 协议与 iOS 应用通信：

- **模拟器端口**: 47164-47169 (localhost)
- **设备端口**: 47175-47179 — 已通过 `iproxy` USB 转发支持
...
所有命令默认连接真实 iOS 应用。会话状态持久化到 `~/.viewglass/session.json`。
...
| 整屏截图 | 可用 | `screenshot screen -o screen.png` |
| 节点截图 | 可用 | `screenshot node <oid> -o node.png` |
| 修改透明度 | 可用 | `attr set <oid> alpha 0.5` |
| 隐藏视图 | 可用 | `attr set <oid> hidden true` |
| 修改文本 | 可用 | `attr set <oid> text "Hello"` |
| 诊断检查 | 可用 | `diagnose all` |
| 发现诊断 | 可用 | `scan --verbose` |

截图说明：
`Viewglass` 现在通过 `ViewglassServer` 高分截图协议统一获取整屏和节点 PNG。
模拟器和真机都走同一条链路，不再依赖 `simctl` 或 `idevicescreenshot`。
...
以独立的 [ViewglassServer](https://github.com/WZBbiao/ViewglassServer)
仓库说明为准完成 iOS App 接入。

这里故意不重复写详细步骤，避免与服务端仓库文档不同步。

只保留几点最重要的信息：
- 只在 `Debug` 中接入 `ViewglassServer`
- 业务代码里仍然使用 `import LookinServer`
- 如果 `viewglass` 找不到 app，先确认 app 里确实启动了 `LookinServer`

## 测试

```bash
swift test     # 运行全部 123 个测试
```
...
与 [Lookin](https://github.com/WZBbiao/viewglass) 相同 — MIT License。
